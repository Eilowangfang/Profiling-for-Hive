...
void extractInittime(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        string::size_type idxl = task[i].logContent.find("Fang: Tez 'Initialization'");
...
void extractInputMap(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        if(idxTask != string::npos) {
            ...
            string::size_type idxStart = task[i].logContent.find("Fang: Tez 'Input' on Map stage starting at time ");
            int rlen = sizeof("Fang: Tez 'Input' on Map stage starting at time ");
            ...
            string::size_type idxEnd = task[i].logContent.find("Fang: Tez 'Input' on Map stage ending at time ");
            rlen = sizeof("Fang: Tez 'Input' on Map stage ending at time ");
            ...
            string sub = "Fang: Tez 'Input' continue to take time ";
            rlen = sizeof("Fang: Tez 'Input' continue to take time ");
...
void extractProcessor(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        if(idxTaskMap != string::npos) {
            ...
            string::size_type idxStart = task[i].logContent.find("Fang: Tez 'Processor' on Map stage starting at time ");
            int rlen = sizeof("Fang: Tez 'Processor' on Map stage starting at time ");
            if(idxStart != string::npos){
                int move = idxStart + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            string::size_type idxEnd = task[i].logContent.find("Fang: Tez 'Processor' on Map stage ending at time ");
            rlen = sizeof("Fang: Tez 'Processor' on Map stage ending at time ");
            if(idxEnd != string::npos){
                int move = idxEnd + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
        ...
        if(idxTaskReduce != string::npos) {
            ...
            string::size_type idxStart = task[i].logContent.find("Fang: Tez 'Processor' on Reduce stage starting at time ");
            int rlen = sizeof("Fang: Tez 'Processor' on Reduce stage starting at time ");
            if(idxStart != string::npos){
                int move = idxStart + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            string::size_type idxEnd = task[i].logContent.find("Fang: Tez 'Processor' on Reduce stage ending at time ");
            rlen = sizeof("Fang: Tez 'Processor' on Reduce stage ending at time ");
            if(idxEnd != string::npos){
                int move = idxEnd + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
...
void extractSink(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        sub[0] = "Fang: Tez 'Sink' takes time ";
        sub[1] = "Fang: Tez 'Sink' at Reduce stage take time ";
        int model = 0;
        int lastPos = -1;
        for(int m = 0; m < 2; m++){
            string tmp = sub[m];
        ...
        if(lastPos > 0){
            ...
            if(model == 0){
                rlen = sizeof("Fang: Tez 'Sink' takes time ");
            }

            if(model == 1){
                rlen = sizeof("Fang: Tez 'Sink' at Reduce stage take time ");
            }
...
void extractSpill(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        if(idxTaskMap != string::npos) {
            ...
            double startCont;
            double endCont;
            string::size_type idxCont = task[i].logContent.find("Fang: Tez 'Spill' on Map stage continuing at time ");
            int rlen = sizeof("Fang: Tez 'Spill' on Map stage continuing at time ");
            ...
            idxCont = task[i].logContent.find("Fang: Tez 'Spill' at Map stage ending at time ");
            rlen = sizeof("Fang: Tez 'Spill' at Map stage ending at time ");
            if(idxCont != string::npos){
                int move = idxCont + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            string sub = "Fang: Tez 'Spill' at Map stage partly take time: ";
            int lastPos = lastPosofStr(task[i].logContent, sub);
            if(lastPos > 0){
                rlen = sizeof("Fang: Tez 'Spill' at Map stage partly take time: ");
                int move = lastPos + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
                }
...
void extractTaskTime(vector<Task>& task){
    ...
    for(int i = 0; i < task.size(); i++){
        ...
        string::size_type idxStart = task[i].logContent.find("Fang: Tez Container task starting at time ");
        int rlen = sizeof("Fang: Tez Container task starting at time ");
        if(idxStart != string::npos){
            int move = idxStart + rlen -1;
            while(task[i].logContent[move] != ' '){
                move++;
        ...
        string::size_type idxEnd = task[i].logContent.find("Fang: Tez Container task ending at time ");
        rlen = sizeof("Fang: Tez Container task ending at time ");
        if(idxEnd != string::npos){
            int move = idxEnd + rlen -1;
            while(task[i].logContent[move] != ' '){
                move++;
...
void extractShuffle(vector<Task>& task){
    for(int i = 0; i < task.size(); i++){
        ...
        if(idxTask != string::npos) {
            ...
            string::size_type idx = task[i].logContent.find("Fang: Tez 'Shuffle' on Reduce stage starting at time ");
            int rlen = sizeof("Fang: Tez 'Shuffle' on Reduce stage starting at time ");
            if(idx != string::npos){
                int move = idx + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            idx = task[i].logContent.find("Fang: Tez 'Shuffle' on Reduce stage ending at time ");
            rlen = sizeof("Fang: Tez 'Shuffle' on Reduce stage ending at time ");
            if(idx != string::npos){
                int move = idx + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
...
void extractOutput(vector<Task>& task){
    for(int i = 0; i < task.size(); i++) {
        ...
        if (idxTask != string::npos) {
            ...
            double startCont;
            double endCont;
            string::size_type idx = task[i].logContent.find("Fang: Tez 'Output' on Reduce stage continuing at time ");
            int rlen = sizeof("Fang: Tez 'Output' on Reduce stage continuing at time ");
            if(idx != string::npos){
                int move = idx + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            idx = task[i].logContent.find("Fang: Tez 'Output' at Reduce stage ending at time ");
            rlen = sizeof("Fang: Tez 'Output' at Reduce stage ending at time ");
            if(idx != string::npos){
                int move = idx + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
            ...
            string sub = "Fang: Tez 'Output' at Reduce stage partly take time: ";
            int lastPos = lastPosofStr(task[i].logContent, sub);
            if(lastPos > 0){
                rlen = sizeof("Fang: Tez 'Output' at Reduce stage partly take time: ");
                int move = lastPos + rlen -1;
                while(task[i].logContent[move] != ' '){
                    move++;
                }
...
int main() {
    FILE *fp = NULL;
    char *file = "/home/spark/demo/fang_workspace/profile_extracter/tmp.log";
    char *line = NULL;
    ...
    while (getline(&line, &line_len, fp) > 0){
        string tmpLine = line;
        string::size_type idxe = tmpLine.find("Fang: Tez Container task ending at time");
        //new task log
        tmpLog += tmpLine;
        if(idxe != string::npos){
            tmpLog += tmpLine;
